 ...
 *
 * CLI:
 *   bun run src/cli/index.ts                    交互式对话
 *   bun run src/cli/index.ts run <workflow.ts>  运行已有 workflow
 *   bun run src/cli/index.ts schedule ...       调度管理
 *   bun run src/cli/index.ts scheduler start    启动调度器
 *   bun run src/cli/index.ts workflows          列出 workflow
 */
...
async function main(): Promise<void> {
	...
	switch (command) {
		case "run": {
			...
			if (!workflowPath) {
				console.error("Usage: bun run src/cli/index.ts run <workflow.ts>");
				process.exit(1);
			}
