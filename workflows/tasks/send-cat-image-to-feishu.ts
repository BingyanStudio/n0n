...
export default async function run() {
  try {
    ...
    if (!imagePathMatch) {
      throw new Error("未能从输出中找到图片保存路径\n输出内容: " + fetchCatOutput);
    }
    ...
    if (sendImageExitCode !== 0) {
      const sendImageOutput = await new Response(sendImageProcess.stdout).text();
      const sendImageError = await new Response(sendImageProcess.stderr).text();
      throw new Error(`发送图片失败，退出码: ${sendImageExitCode}\n输出: ${sendImageOutput}\n错误: ${sendImageError}`);
    }
...
// 自执行入口
if (import.meta.main) {
  run().then((res) => console.log(JSON.stringify(res))).catch((err) => console.error(err));
}