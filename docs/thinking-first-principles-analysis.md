...
**网关 `ai-gateway.wepieoa.com` 实际上支持 Anthropic Messages API！**
...
只需配置 `@ai-sdk/anthropic` 的 `baseURL` 为 `https://ai-gateway.wepieoa.com/v1`，
AI SDK 会拼接为 `{baseURL}/messages` → `https://ai-gateway.wepieoa.com/v1/messages`。

## 用户配置修改

```env
LLM_PROVIDER=anthropic
LLM_BASE_URL=https://ai-gateway.wepieoa.com/v1
