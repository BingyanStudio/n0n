    ...
    case "openai-compatible":
      return {
        ...
        ...(cfg.EDITOR_LLM_BACKEND_PROVIDER
          ? { backendProvider: cfg.EDITOR_LLM_BACKEND_PROVIDER as "anthropic" | "google" | "openai" }
          : {}),
  ...
  const callbacks = {
    onEvent: (round: number, event: StreamEvent) => {
      if (round !== currentRound) {
        currentRound = round;
        roundsDetail.push({ round, events: [], toolResults: [], tokenUsage: null });
        reasoningAccum = "";
      }
      ...
      switch (event.type) {
        ...
        case "tool_call_delta":
          if (event.name) {
            rec.events.push(`tool: ${event.name} (${(event.arguments?.length ?? 0)} chars args)`);
          }
          break;
        case "done":
...
function evaluateExpectation(
  ...
  if (scenario.expectedSuccess && !run.success) {
    issues.push(`预期成功但失败: error="${run.error ?? "无 error, 但 content 未变化"}"`);
  }
  if (!scenario.expectedSuccess && run.success) {
  ...
  if (run.parsedScore !== null) {
    const minOk = scenario.expectedScoreMin !== undefined
      ? run.parsedScore >= scenario.expectedScoreMin
      : true;
    const maxOk = scenario.expectedScoreMax !== undefined
      ? run.parsedScore <= scenario.expectedScoreMax
      : true;
    if (!minOk) {
      issues.push(`分数 ${run.parsedScore} 低于预期最小值 ${scenario.expectedScoreMin}`);
    }
    if (!maxOk) {
      issues.push(`分数 ${run.parsedScore} 高于预期最大值 ${scenario.expectedScoreMax}`);
    }
  }
...
function generateMarkdownReport(
  ...
  for (const preset of presetLabels) {
    ...
    for (const sid of scenarioIds) {
      ...
      const avgScore = sRuns.length > 0
        ? (sRuns.reduce((a, r) => a + (r.parsedScore ?? 0), 0) / sRuns.length).toFixed(1)
        : "N/A";
      const avgRounds = sRuns.length > 0
        ? (sRuns.reduce((a, r) => a + r.rounds, 0) / sRuns.length).toFixed(1)
        : "N/A";
      const avgDuration = sRuns.length > 0
        ? Math.round(sRuns.reduce((a, r) => a + r.durationMs, 0) / sRuns.length)
        : 0;
  ...
  // 预期偏差汇总
  const allIssues: { preset: string; sid: string; run: number; issues: string[] }[] = [];
  ...
  lines.push("| Preset | 场景 | 平均 Input Tokens | 平均 Output Tokens | 平均 Total Tokens | 总计 Total |");
  lines.push("|--------|------|-------------------|--------------------|-------------------|------------|");
  ...
  for (const preset of presetLabels) {
    ...
    for (const sid of scenarioIds) {
      const sRuns = pRuns.filter((r) => r.scenarioId === sid);
      if (sRuns.length === 0) continue;
      const avgInput = Math.round(sRuns.reduce((a, r) => a + r.totalTokenUsage.inputTokens, 0) / sRuns.length);
      const avgOutput = Math.round(sRuns.reduce((a, r) => a + r.totalTokenUsage.outputTokens, 0) / sRuns.length);
      const avgTotal = Math.round(sRuns.reduce((a, r) => a + r.totalTokenUsage.totalTokens, 0) / sRuns.length);
      const sumTotal = sRuns.reduce((a, r) => a + r.totalTokenUsage.totalTokens, 0);
      lines.push(`| ${preset} | ${sid} | ${avgInput.toLocaleString()} | ${avgOutput.toLocaleString()} | ${avgTotal.toLocaleString()} | ${sumTotal.toLocaleString()} |`);
    }
  }

  // 总 cost 估算（按 GPT-4o-mini 费率粗略计算：input $0.15/M, output $0.60/M）
  lines.push("\n### 费用估算（仅供参考）\n");
  const totalInput = allRuns.reduce((a, r) => a + r.totalTokenUsage.inputTokens, 0);
  const totalOutput = allRuns.reduce((a, r) => a + r.totalTokenUsage.outputTokens, 0);
  // 按 GPT-4o-mini 价格估算
...
function saveReport(report: string, presetLabels: string[]) {
...
async function main() {
  ...
  for (const p of presets) {
    console.log(`   - ${p.label} (${p.config.EDIT_BACKEND}, ${p.config.EDITOR_LLM_MODEL})`);
  }
